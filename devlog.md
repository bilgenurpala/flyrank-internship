...
2026-08-20
- Built a robots-aware, rate-limited scraper with bounded backoff, structured JSON output, deterministic parser tests, and explicit ethical limitations.
- Built a reusable SQLite-backed background-job lifecycle with queued, running, done, and failed states; kept retries off until job-specific idempotency exists.
- Added exactly one portfolio feature: a secondary Netlify contact form with native validation, submission feedback, and a honeypot while preserving booking as the primary action.
- Learned that retry safety depends on the side effect, not only the exception; tomorrow: prioritize the SafeBump demo and Checkpoint 2, then reuse the job pattern for BE-07 if time allows.
